...
async function loadPipelineStatus() {
    try {
        ...
        container.innerHTML = `${statusBadge} ${connectedBadge}`;

        // 统计数字 - 紧凑内联
        const s = data.stats || {};
        document.getElementById('monitor-stats-display').innerHTML = `
            <span style="font-size:12px;color:#6b7280;">收到 <strong style="color:#e4e4e7;">${s.total_received || 0}</strong></span>
            <span style="font-size:12px;color:#6b7280;">去重 <strong style="color:#fbbf24;">${s.total_deduped || 0}</strong></span>
            <span style="font-size:12px;color:#6b7280;">意图去重 <strong style="color:#fbbf24;">${s.total_intent_deduped || 0}</strong></span>
            <span style="font-size:12px;color:#6b7280;">分析 <strong style="color:#e4e4e7;">${s.total_analyzed || 0}</strong></span>
            <span style="font-size:12px;color:#6b7280;">转发 <strong style="color:#4ade80;">${s.total_forwarded || 0}</strong></span>
            <span style="font-size:12px;color:#6b7280;">通知 <strong style="color:#4ade80;">${s.total_notified || 0}</strong></span>
        `;
...
const _LEARN_TEMPLATES = {
    ...
    open_position: [{ intent: "open_position", symbol: "BTC-USDT-SWAP", direction: "long", params: { orders: [{ type: "market", leverage: 5, margin_pct: 0.1 }], stop_loss: 105000, take_profit: [110000], move_breakeven: true }, confidence: 0.9 }],
    close_position: [{ intent: "close_position", symbol: "BTC-USDT-SWAP", direction: "long", params: { close_ratio: 0.5 }, confidence: 0.9 }],
    cancel_orders: [{ intent: "cancel_orders", symbol: "BTC-USDT-SWAP", direction: null, params: { cancel_type: "symbol" }, confidence: 0.9 }],
    modify_tp: [{ intent: "modify_tp", symbol: "BTC-USDT-SWAP", direction: null, params: { new_tp: 112000 }, confidence: 0.9 }],
    modify_sl: [{ intent: "modify_sl", symbol: "BTC-USDT-SWAP", direction: null, params: { new_sl: 105000 }, confidence: 0.9 }],
};
